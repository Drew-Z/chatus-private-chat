...
The checked-in and generated configs expose exactly four Durable Object bindings: `USER_STATE -> UserState`, `TEAM_AGENT -> TeamAgent`, `PROVIDER_COORDINATOR -> ProviderCoordinator`, and `INSTANCE_COORDINATOR -> InstanceCoordinator`. Migration tags remain append-only: v1 creates `UserState`, v2 creates `TeamAgent`, v3 creates `ProviderCoordinator`, and v4 creates `InstanceCoordinator`, each through `new_sqlite_classes`. Deployment preparation must preserve all four bindings and migrations; omitting the capture coordinator would let application code compile while every maintenance gate fails closed at runtime.
...
| Checked-in or generated config lacks one of the four Durable Object bindings or v1-v4 SQLite migrations | Fail preflight and dry-run before upload |
...
- Assert the exact four Durable Object bindings, v1-v4 append-only SQLite-backed migrations, and rejection of `new_classes` in both the checked-in and generated Wrangler contracts.
