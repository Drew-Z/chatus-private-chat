...
在不引入未经验证的并发或流式行为变更的前提下，评估并仅在达到性能门槛时缩短本地 Vitest 周期、建立可执行的覆盖率下限，并让成员能够看到有明确定义的 Workspace 元数据占用。
...
- R1. Vitest 必须评估 Node/Workers 显式 project 分池；只有直接或传递依赖 Cloudflare 运行时的测试可进入 Workers project，且 Workers 保持 `maxWorkers: 1`。最终配置仅在 R2 性能门槛通过时保留分池，否则恢复单一 serial Workers pool。
...
- R9. API 类型、测试、React 文件工作区和用户可见文案必须明确这些数字是 metadata-tracked occupancy，而不是 R2 bucket actual usage；新增 `usage` 对象不得暴露对象键、文件路径、checksum 或其他敏感元数据。
- R10. 所有验证只使用本地 fixture、fake Provider/MCP 和 Miniflare；不得调用 live model、真实 MCP/OAuth、production probe 或本地生产部署，并保持 0.x SemVer。

## Acceptance Criteria

- [x] AC1. Node/Workers 分池曾以互斥且完整的 31/9 文件边界接受验证；最终性能门槛失败后已按回滚合同恢复覆盖全部 40 个文件的单一 serial Workers pool，并保留 `maxWorkers: 1`。
- [x] AC2. 最终三次 post-change `npm test` 均通过 40 文件/587 测试，外部 wall time 为 96.900、92.203、88.938 秒，中位数 92.203 秒；因高于 91.564 秒门槛，分池未进入最终配置。
