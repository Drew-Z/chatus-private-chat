...
- After deletion returns or recovers, verify the old cookie up to five times across a 60-second window. HTTP `401` proves revocation. HTTP `200` waits, reissues the idempotent deletion, and checks again because KV `list` may not yet expose a session created seconds earlier. Other statuses, a failed re-delete, or a final `200` fail acceptance.
...
| Old member session remains `200` immediately after deletion | Wait within the 60-second propagation window, reissue idempotent deletion, and require a later `401` |
| Old member session remains `200` after five checks | Fail acceptance; do not claim session revocation |
...
- Unit-test session revocation for immediate `401`, `200 -> wait -> re-delete -> 401`, unexpected status, exhausted `200`, and a re-delete failure whose arbitrary error text is not propagated.
