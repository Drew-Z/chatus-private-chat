...
- Normalize restored selected Skill IDs against the server projection and cap them at three; branches copy the ordered source selection without reapplying create defaults.
- The typed React create path omits `skillIds` for an ordinary new member conversation. The Worker persists up to the first three currently enabled and assigned Skills in registry order; the returned conversation is the only authority for the initial browser selection.
- Conversation hydration only filters the persisted `skillIds` against the current session projection. An empty persisted array remains empty across refresh and switching, while an explicit PATCH empty array remains the member's per-conversation opt-out. Do not apply create defaults during hydration, import, branch, or PATCH.
