...
assert(reactClient.includes("if (busy) return;"), "React client: logout must not abandon an active Agent run");
assert(reactClient.includes("disabled={busy}"), "React client: logout control must be disabled during an active Agent run");
assert(reactClient.includes("await chat.sendMessage({ text })"), "React client: send failures must be observed");
