...
`retryTemporaryMemberDeletion(run, { allowUnauthorized, wait, attempts })` calls an injected status-producing operation sequentially. Status `200` succeeds. Status `401` succeeds only in cleanup mode. Status `503` waits and retries while attempts remain. Every other status and an exhausted `503` produce a stable error containing the operation class and HTTP status, never a body or identifier.
