...
- [x] 加载 file attachment、Agent persistence、deployment specs。
- [x] 增加 R2 binding/deployment config contract 和本地 test binding。
- [x] 先写 schema migration、path normalization、ownership、version pin 与 outbox 测试。
- [x] 实现 Root SQLite tables/upgrader 和文件 domain methods。
- [x] 实现文件 API、R2 object lifecycle、download 和 reconciliation。
- [x] 把 conversation refs 与 send 精确版本读取接入 Agent；保留 inline attachment。
- [x] 实现 conversation/file/account 三层级联清理与失败重试。
