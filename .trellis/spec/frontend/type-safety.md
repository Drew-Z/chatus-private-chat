...
- Legacy-route migration responses use one exact aggregate decoder. `migrated[].providerId` and the matching `results[].providerId` are both present with the same value when a Provider was created, and both absent when an already Provider-backed route only removed stale shadow fields. Reject missing/mismatched presence, mismatched values, duplicate IDs, cross-listing with `alreadyMigrated`, unknown keys, or inconsistent statuses before updating React state.
