...
The checked-in and generated configs expose exactly six Durable Object bindings: `USER_STATE -> UserState`, `TEAM_AGENT -> TeamAgent`, `PROVIDER_COORDINATOR -> ProviderCoordinator`, `INSTANCE_COORDINATOR -> InstanceCoordinator`, `PROVIDER_ATTEMPT_LEDGER -> ProviderAttemptLedger`, and `IDENTITY_REGISTRY -> IdentityRegistry`. Migration tags remain append-only: v1 creates `UserState`, v2 creates `TeamAgent`, v3 creates `ProviderCoordinator`, v4 creates `InstanceCoordinator`, v5 creates `ProviderAttemptLedger`, and v6 creates `IdentityRegistry`, each through `new_sqlite_classes`. Deployment preparation must preserve all six bindings and migrations; omitting either coordinator, the ledger, or the identity registry would let application code compile while maintenance, required Provider execution, or member identity resolution fails closed at runtime. The checked-in baseline sets `PROVIDER_ATTEMPT_LEDGER_MODE="required"`; only exact `disabled` is the reviewed rollback mode.
...
Before generating the production config, GitHub Actions runs `scripts/provision-document-ingest-queues.mjs`. It lists Queue pages without relying on the undocumented `name` query parameter, performs client-side exact `queue_name` matching across every reported page, creates the DLQ before the main Queue only when absent, and repeats the exact lookup after creation. Cloudflare's Queue OpenAPI marks the success-envelope `success`, `errors`, and `messages` fields optional, while the production endpoint also returns `null` for empty `errors/messages`: a 2xx response may omit those fields or use `null`, which the provisioner normalizes to empty arrays. Any present `success` must be boolean, and non-null `errors/messages` must be arrays. The operation-specific `result` remains mandatory (`Queue[]` for list, with post-create exact lookup proving creation). Invalid-envelope diagnostics may expose only the HTTP status and the type/missing state of the fixed `success/errors/messages/result/result_info` fields, never field values or unknown keys. Idempotent GET lookups retry network failures and HTTP 5xx responses at most twice after the initial attempt with 1-second and 3-second delays; 4xx, malformed 2xx envelopes, and write requests are not blindly retried. A failed create is accepted only when a follow-up exact lookup proves a concurrent creator won; authorization, rate-limit, network, malformed-envelope/result/pagination, ambiguous-match, and non-exact verification failures stop deployment after the bounded lookup retries. Queue API credentials remain GitHub Actions secrets and never enter Worker vars or artifacts.
...
| Checked-in or generated config lacks one of the six Durable Object bindings or exact v1-v6 SQLite migrations | Fail preflight and dry-run before upload |
| `PROVIDER_ATTEMPT_LEDGER_MODE` is absent or not exact `required`/`disabled` | Runtime defaults fail closed to `required`; generated production config preserves the reviewed value |
...
| Queue list returns 2xx with a valid `result` and omitted or null empty envelope fields | Normalize null `errors/messages` to empty arrays and continue exact client-side matching |
...
| R2/Queue lookup gets a network error or HTTP 5xx | Retry the idempotent GET at most twice with bounded backoff, then fail closed |
| R2/Queue lookup gets 4xx or malformed 2xx | Fail without retry |
| R2/Queue POST has an uncertain response | Perform one exact follow-up lookup; accept only a proven resource, otherwise preserve the failure without retrying the write |
...
- Good transient recovery: a read-only R2/Queue lookup gets a 5xx, retries with bounded backoff, then succeeds; no POST is repeated blindly.
...
- Bad transient recovery: retry an authorization error or repeat a Queue create after a lost response without an exact follow-up lookup.
...
- Unit-test custom-domain and workers.dev projections, input immutability, KV/R2/Queue binding injection, R2 and Queue provision existing/missing/concurrent/error branches, omitted/null optional Queue envelope fields, empty and multi-page Queue lists, ambiguous exact matches, invalid pagination, bounded network/5xx lookup retries, no retry for 4xx or POST, and removal of stale routes.
...
- Assert the exact six Durable Object bindings, v1-v6 append-only SQLite-backed migrations, required ledger mode, and rejection of `new_classes` in both the checked-in and generated Wrangler contracts.
...
const errors = payload.errors == null ? [] : requireArray(payload.errors);
