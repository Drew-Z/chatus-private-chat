...
async function readyVersion(label: string, root: DurableObjectStub<TeamAgent>, bytes: Uint8Array, name: string, mediaType: string) {
...
describe("document ingest Queue runtime", () => {
  ...
  it("extracts all five queued formats for their exact versions and acknowledges them", async () => {
    const { label, root } = await rootAgent();
  ...
  it("permanently fails active PDF content without retrying", async () => {
    ...
    const { message } = await readyVersion(label, root, bytes, "documents/unsafe.pdf", "application/pdf");
    const queued = queueMessage(message);
  ...
  it("requests exactly three retries before the fourth failed delivery reaches the DLQ", async () => {
    ...
    const { message, reservation } = await readyVersion(label, root, bytes, "notes/missing.txt", "text/plain");
