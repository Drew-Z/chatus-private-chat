...
The checked-in and generated configs expose exactly five Durable Object bindings: `USER_STATE -> UserState`, `TEAM_AGENT -> TeamAgent`, `PROVIDER_COORDINATOR -> ProviderCoordinator`, `INSTANCE_COORDINATOR -> InstanceCoordinator`, and `PROVIDER_ATTEMPT_LEDGER -> ProviderAttemptLedger`. Migration tags remain append-only: v1 creates `UserState`, v2 creates `TeamAgent`, v3 creates `ProviderCoordinator`, v4 creates `InstanceCoordinator`, and v5 creates `ProviderAttemptLedger`, each through `new_sqlite_classes`. Deployment preparation must preserve all five bindings and migrations; omitting either coordinator or the ledger would let application code compile while maintenance or required Provider execution fails closed at runtime. The checked-in baseline sets `PROVIDER_ATTEMPT_LEDGER_MODE="required"`; only exact `disabled` is the reviewed rollback mode.
...
| Checked-in or generated config lacks one of the five Durable Object bindings or exact v1-v5 SQLite migrations | Fail preflight and dry-run before upload |
...
- Assert the exact five Durable Object bindings, v1-v5 append-only SQLite-backed migrations, required ledger mode, and rejection of `new_classes` in both the checked-in and generated Wrangler contracts.
