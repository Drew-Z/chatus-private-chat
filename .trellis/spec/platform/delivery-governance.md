...
use three exact off-round UTC entries: `legacy.api.chat-post` / 30 days at 02:17,
`legacy.browser.shell` / 14 days at 02:27, and `legacy.api.cloud-chats` / 30 days
at 02:37. Manual inputs remain available for bounded bundled-surface
investigation. Manual runs require the
...
Legacy-surface foundation and rollout tasks affect shared Worker, Durable Object,
recovery, and React Operations paths, so they retain both Workspace and local
fake-Provider Agent Playwright evidence in addition to focused state-machine and
...
| A scheduled census uses another surface/window or manual defaults override its cron | Structural governance test fails; do not treat the artifact as per-surface observation evidence |
...
- Structurally assert that both PR browser jobs install Chromium without apt ownership, apply the exact step/download timeouts, and complete installation before invoking their unchanged suite command.
...
- For legacy-surface foundation/rollout changes, also assert the exact bundled
  manifest, phase ceiling, affected surface set, unchanged non-target surfaces,
  recovery evidence, and absence of unsupported cleanup claims.
