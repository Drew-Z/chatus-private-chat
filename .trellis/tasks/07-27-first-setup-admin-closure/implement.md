...
- [x] 加载 frontend/platform specs，确认管理员鉴权与安全投影规则。
- [x] 为 setup-status/smoke 写认证、状态矩阵、敏感字段和零 upstream 测试。
- [x] 实现安全 setup projection 与纯本地 smoke API。
- [x] 扩展 frontend API 类型，新增 AdminSetupGuide 与状态刷新。
- [x] 把六步引导连接到现有 Provider、模型、成员和权限面板。
