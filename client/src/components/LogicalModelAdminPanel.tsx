...
export function LogicalModelAdminPanel({
  ...
  function selectModel(id: string) {
    if (busy) return;
    if (dirty && !window.confirm("放弃当前未保存的逻辑模型修改？")) return;
    if (id === "__new__") {
  ...
  async function deleteModel() {
    ...
    if (!window.confirm(`删除逻辑模型「${selectedModel.label}」？`)) return;
    setBusy(true);
    try {
      const routes = { ...snapshot.config.routes };
      delete routes[selectedModel.id];
  ...
  return (
    <section className="admin-pool-panel" aria-labelledby="logical-model-admin-title">
      <div className="admin-pool-sidebar">
        <div className="admin-pool-sidebar-head"><div><p className="eyebrow">LOGICAL MODELS</p><h1 id="logical-model-admin-title">逻辑模型</h1></div><button className="icon-button" type="button" onClick={() => selectModel("__new__")} disabled={busy} aria-label="新增逻辑模型" title="新增逻辑模型"><Plus size={17} /></button></div>
